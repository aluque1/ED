...
sol max_interes_ruta(const BinTree<int> &montanya) {
  if (montanya.empty()) {
    return {0, 0};
  } else if (montanya.left().empty() && montanya.right().empty()) {
    return {montanya.root(), montanya.root()};
  } else {
    sol iz = max_interes_ruta(montanya.left());
    sol dr = max_interes_ruta(montanya.right());

    return { montanya.root() + max(iz.curr, dr.curr), max(max(iz.max, dr.max), montanya.root() + iz.curr + dr.curr)};
  }
}
...
bool tratar_caso() {
  ...
  cout << max_interes_ruta(arbol).max << endl;
  return true;
}
